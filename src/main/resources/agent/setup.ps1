...
New-Item -ItemType Directory -Force -Path $root,$dir,$recoveryDir,$stage | Out-Null
...
function Download-Retry([string]$uri,[string]$out,[long]$min,[string]$label){
  ...
  for($i=1;$i -le 4;$i++){
    try{
      Remove-Safe $out
      Write-Host ($label+' yüklənir... '+$i+'/4')
      Invoke-WebRequest -UseBasicParsing -TimeoutSec 120 -Uri $uri -OutFile $out
      if(-not (Test-Path $out)){throw ($label+' faylı yaranmadı.')}
      if((Get-Item $out).Length -lt $min){throw ($label+' natamam yükləndi.')}
      return
    }catch{$last=$_.Exception.Message;Remove-Safe $out;Log ($label+' yükləmə cəhdi '+$i+' alınmadı: '+$last);if($i -lt 4){Start-Sleep -Seconds (2*$i)}}
  }
...
function Find-PrivateJava21([string]$baseDir){
  ...
  $releases=@(Get-ChildItem -LiteralPath $baseDir -Recurse -Filter release -File -ErrorAction SilentlyContinue)
  foreach($release in $releases){$probe=Get-Java21Probe $release.Directory.FullName;if($probe.Valid){return [pscustomobject]@{Launch=$probe.Launch;Check=$probe.Check;Home=$probe.Home}}}
  $javas=@(Get-ChildItem -LiteralPath $baseDir -Recurse -Filter java.exe -File -ErrorAction SilentlyContinue|Where-Object{$_.FullName -match '\\bin\\java\.exe$'})
  foreach($java in $javas){$javaHomeCandidate=Split-Path -Parent (Split-Path -Parent $java.FullName);$probe=Get-Java21Probe $javaHomeCandidate;if($probe.Valid){return [pscustomobject]@{Launch=$probe.Launch;Check=$probe.Check;Home=$probe.Home}}}
  return $null
}
...
function Wait-Process{
  for($i=0;$i -lt 28;$i++){try{$s=Invoke-RestMethod -UseBasicParsing -TimeoutSec 2 -Headers $agentHeaders -Uri 'http://127.0.0.1:47631/api/status';if($s.ok -eq $true -and [string]$s.version -eq $ExpectedVersion){return $true}}catch{};Start-Sleep -Milliseconds 750};return $false
}
function Wait-Complete{
  for($i=0;$i -lt 20;$i++){try{$s=Invoke-RestMethod -UseBasicParsing -TimeoutSec 2 -Headers $agentHeaders -Uri 'http://127.0.0.1:47631/api/status';if($s.installComplete -eq $true -and [string]$s.version -eq $ExpectedVersion){return $true}}catch{};Start-Sleep -Milliseconds 600};return $false
}
...
try{
  ...
  # Manual/automatic repair always stages a clean server copy. The currently working
  # installation stays untouched until every staged component passes validation.
  Download-Retry ($Base+'/agent/taxdata-agent.jar') $stagedJar 100000 'TaxData Agent'
  ...
  if($systemJava){$javaLaunch=$systemJava.Launch;$javaCheck=$systemJava.Check}
  elseif($existingPrivate){$javaMode='private';$javaLaunch=$existingPrivate.Launch;$javaCheck=$existingPrivate.Check;Write-Host 'Mövcud private Java 21 bütövdür.'}
  else{
    $javaMode='private';$replaceJre=$true
    $jreZip=Join-Path $stage 'jre.zip';$stagedJre=Join-Path $stage 'jre'
    $runtimeArch='x64'
    try{$osArch=[string](Get-CimInstance Win32_OperatingSystem -ErrorAction Stop).OSArchitecture;if($osArch -match '(?i)ARM'){$runtimeArch='aarch64'}}catch{if($env:PROCESSOR_ARCHITECTURE -match '(?i)ARM64' -or $env:PROCESSOR_ARCHITEW6432 -match '(?i)ARM64'){$runtimeArch='aarch64'}}
    ...
    Expand-Archive -Force $jreZip $stagedJre;Remove-Safe $jreZip
    $j=Find-PrivateJava21 $stagedJre;if(-not $j){$candidate=Find-FirstJavaCandidate $stagedJre;if($candidate){Show-JavaProbe (Get-Java21Probe $candidate)};throw 'Yüklənən Java runtime Java 21 yoxlamasından keçmədi.'}
    $javaLaunch=$j.Launch;$javaCheck=$j.Check
  }

  $stagedRepair=Join-Path $stage 'TaxData-Agent-Repair.cmd'
  Download-Retry ($Base+'/agent/installer') $stagedRepair 500 'TaxData bərpa quraşdırıcısı'
...
"@
  ...
  if($replaceJre){try{Get-ChildItem -LiteralPath $finalJre -Recurse -File -ErrorAction SilentlyContinue|Unblock-File -ErrorAction SilentlyContinue}catch{}}
...
function Start-Agent{
  ...
  if(Test-Path `$start){try{Start-Process -WindowStyle Hidden -FilePath 'powershell.exe' -ArgumentList @('-NoProfile','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File',`$start)}catch{RLog ('PowerShell fallback start alınmadı: '+`$_.Exception.Message)}}
}
function Full-Repair([bool]`$forUpdate=`$false){
  ...
  try{
    ...
    `$p=Start-Process -WindowStyle Hidden -Wait -PassThru -FilePath 'powershell.exe' -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',`$tmp)
...
if(`$restartAfterUpdate){
  Start-Sleep -Seconds 2
  try{Start-Process -WindowStyle Hidden -FilePath 'powershell.exe' -ArgumentList @('-NoProfile','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File',`$watchdog)}catch{}
}
"@
  ...
  Start-Process -WindowStyle Hidden -FilePath 'powershell.exe' -ArgumentList @('-NoProfile','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File',$watchdog)
...
"@
  ...
  if($commitStarted){
    ...
    if(Test-Path $start){try{Start-Process -WindowStyle Hidden -FilePath 'powershell.exe' -ArgumentList @('-NoProfile','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File',$start)}catch{}}
    if(Test-Path $watchdog){try{Start-Process -WindowStyle Hidden -FilePath 'powershell.exe' -ArgumentList @('-NoProfile','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File',$watchdog)}catch{}}
  }
