...
public class OutgoingInvoiceService {
    ...
    public Map<String,Object> packageInvoices(String ws,List<String> ids) throws Exception {
        ...
        for(OutgoingInvoice q:selected){String name=safeFile(q.getInvoiceNumber())+".xml";Files.write(dir.resolve(name),xml(q));xmlNames.add(name);q.setStatus("PACKAGED");q.setUpdatedAt(LocalDateTime.now());}
...
""".strip();
    ...
    private String xmlEscape(String value){String s=safe(value);StringBuilder out=new StringBuilder(s.length()+16);for(int i=0;i<s.length();){int cp=s.codePointAt(i);i+=Character.charCount(cp);if(!(cp==0x9||cp==0xA||cp==0xD||(cp>=0x20&&cp<=0xD7FF)||(cp>=0xE000&&cp<=0xFFFD)||(cp>=0x10000&&cp<=0x10FFFF)))continue;switch(cp){case '&'->out.append("&amp;");case '<'->out.append("&lt;");case '>'->out.append("&gt;");default->out.appendCodePoint(cp);}}return out.toString();}
